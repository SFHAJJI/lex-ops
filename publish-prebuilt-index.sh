...
git merge-base --is-ancestor "$QUEUE_COMMIT" refs/remotes/origin/fleet-status \
  || { echo "ERROR: queue commit is not on the generated fleet-status history" >&2; exit 2; }
...
git -C lex merge-base --is-ancestor "$BUILD_CODE_COMMIT" refs/remotes/origin/main \
  || { echo "ERROR: ticketed Lex commit is not on main" >&2; exit 2; }
...
git -C corpus merge-base --is-ancestor "$CORPUS_COMMIT" refs/remotes/origin/main \
  || { echo "ERROR: ticketed corpus commit is not on main" >&2; exit 2; }
...
git -C articles-ticket merge-base --is-ancestor "$ARTICLES_COMMIT" refs/remotes/origin/main \
  || { echo "ERROR: ticketed articles commit is not on main" >&2; exit 2; }
